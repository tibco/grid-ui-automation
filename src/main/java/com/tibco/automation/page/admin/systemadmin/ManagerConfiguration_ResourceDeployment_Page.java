...
public class ManagerConfiguration_ResourceDeployment_Page extends TemplatePage implements Locators.ManagerConfiguration_ResourceDeploymentPageLocators{
	...
	public ManagerConfiguration_ResourceDeployment_Page() {
	...
	public void launchResourceDeploymentPage()
	{
		driver.getAssertionService().addAssertionLog("Launching Resource Deployment page", MessageTypes.Info);
		new ExtendedWebElement(RESOURCE_DEPLOYMENT_SIDE_MENU_LOC).click();
		driver.getWaitUtility().waitForElementPresent(EXPAND_COLLAPSE_VERIFICATION_TEXT_LOC);
		driver.getAssertionService().assertTrue(new ExtendedWebElement(EXPAND_COLLAPSE_VERIFICATION_TEXT_LOC).isPresent(), "Resource Deployment page is not launched", "Resource Deployment page is launched");
	}
	...
	public void expandAll()
	{
		...
		driver.getWaitUtility().waitForElementVisible(SIGN_IF_UNMODIFIED_SELECT_LOC);
		driver.getAssertionService().assertTrue(new ExtendedWebElement(SIGN_IF_UNMODIFIED_SELECT_LOC).isDisplayed(), "Expand all button is not clicked", "Expand all button is clicked");
	}
	...
	public void collapseAll()
	{
		...
		driver.getAssertionService().assertFalse(new ExtendedWebElement(SIGN_IF_UNMODIFIED_SELECT_LOC).isDisplayed(), "Collapse all button is not clicked", "Collapse all button is clicked");
	}
	...
	{
		Select logLevelSelect=new Select(new ExtendedWebElement(SIGN_IF_UNMODIFIED_SELECT_LOC));
		logLevelSelect.selectByValue(value);
		WebElement option=logLevelSelect.getFirstSelectedOption();
		driver.getAssertionService().assertTrue((option.getText()).equalsIgnoreCase(value), "Sign if unmodified is not set to "+value, "Sign if unmodified is set to "+value);
	}
	...
	{
		new ExtendedWebElement(COMMON_HEADER_LOC).click();
	...
	{
		driver.getWaitUtility().waitForElementPresent(TOKEN_TIMEOUT_DESCRIPTION_TOOLTIP);
		driver.getAssertionService().assertTrue(new ExtendedWebElement(TOKEN_TIMEOUT_DESCRIPTION_TOOLTIP).isDisplayed(), "Tooltip for description is not present", "Tooltip for description is present");
	}
	
	public void verifyNumericDataTooltip()
	{
		driver.getWaitUtility().waitForElementPresent(DIRECTOR_MONITOR_INTERVAL_TOOLTIP_LOC);
		driver.getAssertionService().assertTrue(new ExtendedWebElement(DIRECTOR_MONITOR_INTERVAL_TOOLTIP_LOC).isDisplayed(), "Tooltip for numeric data is not present", "Tooltip for numeric data is present");
	}
}
