...
public class BrokerMonitorPage extends TemplatePage implements Locators,Locators.BrokerMonitorPageLocators{
	...
	public void clickBrokerMonitorLink()
	{
		driver.getWaitUtility().waitForElementPresent(BROKER_MONITOR_LINK_LOC);
		new ExtendedWebElement(BROKER_MONITOR_LINK_LOC).click();
		driver.getAssertionService().addAssertionLog("Clicking broker monitor link", MessageTypes.Info);
	}
	...
	public void verifyAllCheckboxesAreChecked()
	{
		...
		String checkedTaskStatus=new ExtendedWebElement(TASKS_CHECKBOX_LOC).getAttribute("checked");
		String checkedServicesStatus=new ExtendedWebElement(SERVICE_VIEW_CHECKBOX_LOC).getAttribute("checked");
		...
		if (checkedEngineStatus.equalsIgnoreCase("true") && checkedTaskStatus.equalsIgnoreCase("true") && checkedServicesStatus.equalsIgnoreCase("true") && checkedSystemStatus.equalsIgnoreCase("true"))
			driver.getAssertionService().addAssertionLog("All checkboxes except services are checked by default", MessageTypes.Pass);
		else
	...
	public void verifyAllGraphsArePresent()
	{
		...
		driver.getAssertionService().assertElementPresent(By.xpath(SERVICE_VIEW_GRAPH_LOC), "Service View Graph");
	...
	{
		driver.getWaitUtility().waitForElementPresent(TASKS_CHECKBOX_LOC);
		new ExtendedWebElement(TASKS_CHECKBOX_LOC).click();
	...
	public void clickServiceViewCheckBox()
	{
		driver.getWaitUtility().waitForElementPresent(SERVICE_VIEW_CHECKBOX_LOC);
		new ExtendedWebElement(SERVICE_VIEW_CHECKBOX_LOC).click();
		driver.getAssertionService().addAssertionLog("Services checkbox is clicked", MessageTypes.Info);
	}
	...
	public void verifyServiceViewGraphIsPresent()
	{
		driver.getWaitUtility().waitForElementPresent(SERVICE_VIEW_GRAPH_LOC);
		driver.getAssertionService().assertElementPresent(By.xpath(SERVICE_VIEW_GRAPH_LOC), "Service view Graph");
	}
	
	
	public void verifyServiceViewGraphIsNotPresent()
	{
		driver.getWaitUtility().waitForElementNotPresent(SERVICE_VIEW_GRAPH_LOC);
		driver.getAssertionService().assertElementNotPresent(By.xpath(SERVICE_VIEW_GRAPH_LOC), "Service view Graph");
	}
	...
	public void verifyServiceGraphIsNotPresent()
	{
		driver.getWaitUtility().waitForElementNotPresent(SERVICES_GRAPH_LOC);
		driver.getAssertionService().assertElementNotPresent(By.xpath(SERVICES_GRAPH_LOC), "Service Graph");
	}
	
	
	public void verifyServiceGraphIsPresent()
	{
